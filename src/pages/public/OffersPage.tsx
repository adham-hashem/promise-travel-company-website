...
import { Tag, Loader2, Clock, ArrowLeft, Star, Link2, CheckCircle2 } from 'lucide-react';
          ...
          {loading ? (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filtered.map((o) => {
                ...
                const offerUrl = `${window.location.origin}/#offer/${o.id}`;
                return (
                  <div key={o.id} className="public-card overflow-hidden relative cursor-pointer group" onClick={() => onNavigate('offer-details', undefined, o.id)}>
                    ...
                    <div className="absolute top-4 left-4 z-10" onClick={(e) => e.stopPropagation()}><ShareButton title={o.name} compact shareUrl={offerUrl} /></div>
                    ...
                    <div className="p-5">
                      ...
                      <div className="flex gap-2">
                        <button onClick={(e) => { e.stopPropagation(); onNavigate('offer-details', undefined, o.id); }} className="flex-1 bg-emerald-950 text-white font-bold text-xs px-3 py-2.5 rounded-xl hover:bg-emerald-900 transition-all">عرض التفاصيل</button>
                        <button onClick={(e) => { e.stopPropagation(); onNavigate('booking', { packageId: o.package_id || o.packages?.id, type: offerType }); }} className="flex-1 bg-gradient-gold text-emerald-950 font-bold text-xs px-3 py-2.5 rounded-xl hover:shadow-lg transition-all">احجز الآن</button>
                      </div>
                      <CopyLinkButton url={offerUrl} />
                    </div>
                  </div>
                );
              })}
