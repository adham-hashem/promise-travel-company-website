...
export default function OfferDetailsPage({ offerId, onNavigate }: Props) {
  ...
  return (
    <div className="bg-[#fbfaf7] pb-20">
      ...
      <section className="max-w-7xl mx-auto px-4 pt-3"><div className="relative h-[360px] md:h-[510px] rounded-[2rem] overflow-hidden"><img src={image} alt={offer.name} className="w-full h-full object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-emerald-950/90 via-emerald-950/30 to-transparent" /><div className="absolute top-6 right-6 bg-red-600 text-white font-black rounded-xl px-4 py-2">خصم {offer.discount_percentage}%</div><div className="absolute top-6 left-6"><ShareButton title={offer.name} compact /></div><div className="absolute bottom-8 right-6 left-6 text-white"><span className="inline-flex items-center gap-2 text-gold-300 text-sm font-bold mb-3"><Tag size={15} /> {type}</span><h1 className="text-3xl md:text-5xl font-black">{offer.name}</h1><p className="text-white/75 mt-3 max-w-2xl">{offer.description || 'استمتع بتجربة سفر متكاملة مع PROMISE وخدمة تهتم بكل التفاصيل.'}</p></div></div></section>
      <section className="max-w-7xl mx-auto px-4 mt-8 grid lg:grid-cols-[1fr_360px] gap-8">
        <div className="space-y-7">
          <div className="grid sm:grid-cols-3 gap-3"><div className="public-card p-5"><Clock3 size={20} className="text-gold-600 mb-3" /><p className="text-xs text-gray-400">المدة</p><p className="font-black text-emerald-950 mt-1">{pkg?.duration_days || 'حسب البرنامج'} {pkg?.duration_days ? 'أيام' : ''}</p></div><div className="public-card p-5"><MapPin size={20} className="text-gold-600 mb-3" /><p className="text-xs text-gray-400">الوجهة</p><p className="font-black text-emerald-950 mt-1">{pkg?.destination || (type === 'حج' || type === 'عمرة' ? 'مكة والمدينة' : 'وجهة مميزة')}</p></div><div className="public-card p-5"><CalendarDays size={20} className="text-gold-600 mb-3" /><p className="text-xs text-gray-400">صلاحية العرض</p><p className="font-black text-emerald-950 mt-1 text-sm">حتى {new Date(offer.end_date).toLocaleDateString('ar-EG')}</p></div></div>
